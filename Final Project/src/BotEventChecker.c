...
static uint16_t lastParam = 0b0000;
static uint16_t lastBump = 0x0000;
...
uint8_t CheckTape(void) {
    ...
    ES_Event thisEvent;
    uint8_t returnVal = FALSE;
    uint16_t tapeValue = botReadTape(); // read the tape sensors
    //printf("\r\n tapevals: %d", tapeValue);


    if (tapeValue == 0b0000) { // are any tape sensors triggered?
        curEvent = TAPE_NOT_SENSED;
    } else {
        curEvent = TAPE_SENSED;
    }
    if (curEvent != lastEvent | tapeValue != lastParam) { // check for change from last time
        thisEvent.EventType = curEvent;
        thisEvent.EventParam = tapeValue;
        returnVal = TRUE;
        lastEvent = curEvent; // update history
        lastParam = tapeValue;
#ifndef EVENTCHECKER_TEST           // keep this as is for test harness
...
uint8_t CheckWall(void) {
    ...
    ES_Event thisWall;
    uint8_t returnVal = FALSE;
    uint8_t wallValue = wallTape();

    if (wallValue == 1) {
        curWall = WALL_NOT_FOUND;
    } else {
        curWall = WALL_FOUND;
    }
    if (curWall != lastWall) {
        thisWall.EventType = curWall;
        returnVal = TRUE;
        lastWall = curWall;
#ifndef EVENTCHECKER_TEST           // keep this as is for test harness
        //PostTemplateService(thisEvent);
...
uint8_t CheckOtherWall(void) {
    ...
    ES_Event thisWall;
    uint8_t returnVal = FALSE;
    uint8_t wallValue = otherWallTape();

    if (wallValue == 1) {
        curWall = OTHER_WALL_NOT_FOUND;
    } else {
        curWall = OTHER_WALL_FOUND;
    }
    if (curWall != lastWall) {
        thisWall.EventType = curWall;
        returnVal = TRUE;
